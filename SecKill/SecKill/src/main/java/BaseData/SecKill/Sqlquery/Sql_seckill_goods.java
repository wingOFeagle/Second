...
//拉取品牌表的数据
public class Sql_seckill_goods
{
	private Connection m_sqlconn = null;
	private Statement m_stmt;
	private static Logger log = Logger.getLogger(Sql_seckill_goods.class);

	public Sql_seckill_goods()
	{
		log.info("begin Sql_seckill_goods");
		// 初始化sql对象
		...
		try
		{
			...
			log.error("Sql_seckill_goods 成功加载MySQL驱动程序");
			// 一个Connection代表一个数据库连接
			m_sqlconn = DriverManager.getConnection(url);
			// Statement里面带有很多方法，比如executeUpdate可以实现插入，更新和删除等
			if (m_sqlconn == null)
			{
				log.error("Sql_seckill_goods m_sqlconn is null");
			}
			m_stmt = m_sqlconn.createStatement();
		} catch (SQLException e)
		{
			log.error("Sql_seckill_goods MySql操作错误");
			e.printStackTrace();
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public void UpdateData()
	{
		TimerTask task = new TimerTask()
		{
			...
			public void run()
			{
				...
				try
				{
					log.info("Begin to query data in Sql_seckill_goods");
					String time_now = DateInterface.getNowDate();
					String sql = String.format("select sku_id,sku_name,jd_price,promo_price,start_date,end_date from cms_seckill_goods where status=1 and start_date <= '%s' and end_date >= '%s';",time_now,time_now);
