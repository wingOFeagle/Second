...
public class OrderamountexpandConsumeOld extends Thread
{
	...
	public OrderamountexpandConsumeOld(RedisInterface redisInterface)
	{
	...
	private void HandleMsg(String key, JdwData data, long offset, int partition)
	{
		...
		try
		{
			...
			if (MapCache.GetMapCache().containsKey(m_strSkuid) && m_mapType.containsKey(m_strType))// 判断skuid的有效性,且type在选定的type之中
			{
				// 判断是否在redis中
				...
				{
					m_StatusInRedis = m_redisInterface.GetValueInRedis(jedis, m_strOrderid_Skuid_key, "Status");
					m_hasUsedInRedis = m_redisInterface.GetValueInRedis(jedis, m_strOrderid_Skuid_key, "hasUsed");

					// 默认新消息都是有效的，所以基本上不需要判断现有消息的status数值
					if (m_StatusInRedis == "1")// 原redis中存储的status状态
					{
						if (m_hasUsedInRedis == "0")// 该订单尚没有使用过
							UpdateSkuOrder_valid(jedis);// 用来更新sku对应订单数值
						else if (m_hasUsedInRedis == "0")// hasUsed已经用过，但是依然有优惠，应该取消掉原订单
						{
							SaveSkuTotalMinus(jedis);// 从sku的维度更新redis
							SaveSkuTimeToalMinus(jedis);// 从sku+time的维度更新redis
					...
					else if (m_StatusInRedis == "0")
						UpdateSkuOrder_invalid(jedis);// 把一些字段保存起来，保存redis,只是进行保存，并不进行逻辑处理
					else
						log.error("OrderamountexpandConsume: status_redis is " + m_StatusInRedis + ", it is not 1 or 0");
				}
				else
	...
	private void UpdateSkuOrder_invalid(Jedis jedis)
	{
		...
		jedis.hset(m_strOrderid_Skuid_key, m_strType_Amount_key, m_strAmount);
	...
	private void SaveSkuTotalAdd(Jedis jedis, String Promo_price)
	{
		try
		{
			...
			if (!jedis.hexists(m_strSkuid, "Promo_price"))// 不存在的时候再进行存储
				RedisInterface.getM_pool().getResource().hset(m_strSkuid, "Promo_price", Promo_price);// 保存数据至Redis
		} catch (Exception e)
		{
	...
	// 以Skuid为维度，进行存储
	private void SaveSkuTotalMinus(Jedis jedis)
	{
		try
		{
			// 进行秒杀订单的存储
			...
			String TotalNumNew = String.valueOf(Double.parseDouble(TotalNum) - Double.parseDouble(m_strNum));
			// 开始保存
			jedis.hset(m_strOrderid_Skuid_key, "Status", "0");// 订单使用,现在m_strStatusNow为"0",订单取消状态
			jedis.hset(m_strSkuid, "TotalMoney", TotalMoneyNew);// 保存数据至Redis
			jedis.hset(m_strSkuid, "TotalNum", TotalNumNew);// 保存数据至Redis
		} catch (Exception e)
		{
			log.error("OrderamountexpandConsume: SaveSkuTotal error" + e);
		}
	}
	...
	private void SaveSkuTimeToalAdd(Jedis jedis,String Promo_price)
	{
		...
		{
			log.error("OrderamountexpandConsume: SaveSkuTimeToal error," + e);
		} 
	}

	// 以Skuid+Time,进行存储
	private void SaveSkuTimeToalMinus(Jedis jedis)
	{
		try
		{
			Date date = new Date(m_strCreatetime);// 更新某个时间段里某个skuid的订单状况
