...
public class OrderamountexpandConsume extends Thread
{
	...
	private static Logger log = Logger
			.getLogger(OrderamountexpandConsume.class);
	...
	public void run()
	{
		try
		{
			...
			System.out.println("OrderamountexpandConsume开始执行");
			Consume();
	...
	private void HandleMsg(String key, JdwData data, long offset,int partition)
	{
		...
		BusinessInfo.INFO.put(BusinessInfo.KEY_RECEIVE_ORDERAMOUNT_EXPAND, KeyType.CUSTOM, 1,false);
	}
	 ...
	 * 
	 * public String fromBytes(byte[] bytes) { String value = null; try { value
	 * = new String(bytes, "utf-8"); } catch (UnsupportedEncodingException e) {
