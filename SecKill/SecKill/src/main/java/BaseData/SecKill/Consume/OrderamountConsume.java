...
public class OrderamountConsume extends Thread
{
	...
	private String m_strSkuid, m_strOrderid, m_strMoneyBalance, m_strNum, m_strCreatetime, m_strPrice, m_strStatusNow, m_stramountPayable;
	...
	public OrderamountConsume(RedisInterface redisInterface)
	{
	...
	private void HandleMsg(String key, JdwData data, long offset, int partition)
	{
		...
		try
		{
			...
			m_bCreatetime = false;// 每次循环初始值都为false
			String value = data.toString();
			InitMsg(value);// 解析消息数据流
			if (MapCache.GetMapCache().containsKey(m_strSkuid))// 判断skuid的有效性
			{
				// 判断是否在redis中
	...
	{
		// 把一些字段保存起来，保存redis,只是进行保存，并不进行逻辑处理
		jedis.hset(m_strOrderid_Skuid_key, "MoneyBalance", m_strMoneyBalance);
		jedis.hset(m_strOrderid_Skuid_key, "amountPayable", m_stramountPayable);
		jedis.hset(m_strOrderid_Skuid_key, "Createtime", m_strCreatetime);
		jedis.hset(m_strOrderid_Skuid_key, "Num", m_strNum);
		jedis.hset(m_strOrderid_Skuid_key, "Price", m_strPrice);
	...
	private void UpdateSkuOrder_valid(Jedis jedis)
	{
		...
		String ResultNew = String.valueOf(Double.parseDouble(m_strPrice) + Double.parseDouble(ResultInRedis));
		...
		if (m_bCreatetime)// 只有在orders表中查询到对应的订单时候才会进行更新
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "Createtime", m_strCreatetime, true);
			//jedis.hset(m_strOrderid_Skuid_key, "Createtime", m_strCreatetime);// 保存数据至Redis
		// 从内存中查询skuid对应的信息
		...
		for (String Promo_price : Promo_price_array.split(","))
		{
			if (PriceCompare == Promo_price)// 价格是促销价格则保存，否则不做什么
			{
				...
				SaveSkuidToRedis(jedis);// 将确定为秒杀订单的skuid进行记录
				break;
			}
		}
	}
	...
	private void InitMsg(String value)
	{
		try
		{
			...
			if (opt == "INSERT")
			{
				...
				m_strMoneyBalance = js_obj.getString("MoneyBalance");
				m_stramountPayable = js_obj.getString("amountPayable");
				m_strNum = js_obj.getString("Num");
				m_strCreatetime = js_obj.getString("Createtime");// 这里的创建时间应该按照orderamount这张表里的来计算
				m_strPrice = js_obj.getString("Price");
			}
			else if (opt == "UPDATE")
				log.error("OrderamountConsume: opt is " + opt);
			else
				log.error("OrderamountConsume: opt is " + opt + ", not INSERT or UPDATE");
		} catch (Exception e)
		{
			log.error("OrderamountConsume : InitMsg" + e);
		}
	}
	...
	private void InitStore(Jedis jedis)
	{
		...
		{
			// 把一些初始化的字段保存起来至redis
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "MoneyBalance", m_strMoneyBalance, true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "amountPayable", m_stramountPayable, true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "Createtime", m_strCreatetime, true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "Num", m_strNum, true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "Price", m_strPrice, true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "Result", m_strPrice, true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "Status", "1", true);
			m_redisInterface.SetKeyValue(jedis,m_strOrderid_Skuid_key, "StathasUsedus", "0", true);
			/*jedis.hset(m_strOrderid_Skuid_key, "MoneyBalance", m_strMoneyBalance);
			jedis.hset(m_strOrderid_Skuid_key, "amountPayable", m_stramountPayable);
			jedis.hset(m_strOrderid_Skuid_key, "Createtime", m_strCreatetime);// 更新时间
			jedis.hset(m_strOrderid_Skuid_key, "Num", m_strNum);
			jedis.hset(m_strOrderid_Skuid_key, "Price", m_strPrice);
			jedis.hset(m_strOrderid_Skuid_key, "Result", m_strPrice);
			jedis.hset(m_strOrderid_Skuid_key, "Status", "1");
			jedis.hset(m_strOrderid_Skuid_key, "hasUsed", "0");// 未被使用过
*/		} catch (Exception e)
		{
			log.error("OrderamountConsume:InitStore error " + e);
		}
	}
